...
class _QuoteAcceptConfirmScreenState extends State<QuoteAcceptConfirmScreen> {
  ...
  void _handleConfirmOrder() {
    ...
    final newOrder = AppOrder(
      ...
      productTitle: request.productType,
      productImage: request.referenceImages?.firstOrNull ?? 'https://picsum.photos/seed/custom/400/400',
      vendorId: 'v_custom',
      vendorName: 'Hunarmand Custom Shop',
      vendorVerified: true,
      quantity: request.quantity,
      pricePerUnit: (request.quote!.totalPrice / request.quantity).round(),
      totalAmount: request.quote!.totalPrice,
      status: 'confirmed',
    ...
    context.read<CustomRequestProvider>().acceptQuote(widget.requestId);

    // Navigate to shared Order Confirmation Screen
    context.go('/order-confirmation/$newOrderId');
  }
}
